...
public class RandomForest extends Classification {
	...
	public RandomForest(int max_level, int min_datacnt, double ratio) {
		MAX_LEVEL = max_level;
	...
	private Split findSplit(LinkedList<Feature> data) {
		Split res = null;
		double best = 1e100;
	...
	private int learn(LinkedList<Feature> data, int depth) {
		int id = tree.size();
...
//		System.out.println(data.size()+"\t"+cur.result);
		...
		Split cut = null;
		if (depth<MAX_LEVEL) cut=findSplit(data);
		if (cut != null) {
			LinkedList<Feature> dataleft = new LinkedList<Feature>();
			LinkedList<Feature> dataright = new LinkedList<Feature>();
			for (Feature item : data){
			...
			cur.split = cut;
			System.out.println(depth+"\t"+cur.result+"\t"+dataleft.size()+"\t"+dataright.size()+"\t"+cur.split);
...
class TreeNode {
	...
	public String toString() {
		return depth+"\t"+cnt+"\t"+result+"\t"+left+"\t"+right+"\t"+split;
	}
}
